...
# MAGIC %sql
# MAGIC select * from table_changes('silverTable',2,3)
...
# 
# changes_df = spark.read.format("delta").option("readChangeData", True).option("startingVersion", 2).
# table('silverTable')
# display(changes_df)
...
# MAGIC %sql
# MAGIC create or replace temp view silver_table_view as
# MAGIC select * from 
# MAGIC (select *,rank() over(partition by Country order by _commit_version) as rank
# MAGIC  from table_changes('silverTable',2) where _change_type!='update_preimage') where rank=1
